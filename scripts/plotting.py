...
def report_gmean_and_max_speedup(policy_data, baseline_data) -> float:
    policy_ipc = get_stat_for_workloads(policy_data, 'IPC')
    baseline_ipc = get_stat_for_workloads(baseline_data, 'IPC')

    speedup = []
    for workload in policy_ipc:
        speedup.append(policy_ipc[workload] / baseline_ipc[workload])
    return sp.stats.gmean(speedup), max(speedup)

##############################################################################
##############################################################################

## BARD SPECIFIC FUNCTIONS:
