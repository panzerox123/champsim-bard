...
std::vector<std::string> champsim::plain_printer::format(DRAM_CHANNEL::stats_type stats)
{
  ...
  lines.push_back(fmt::format("  WRITE OCCU PRE DRAIN: {:10}\tPOST DRAIN: {:10}", mean(stats.tot_write_occu_pre_drain, stats.num_write_drains), mean(stats.tot_write_occu_post_drain, stats.num_write_drains)));
  lines.push_back(fmt::format("  MEAN BLP: {:10}\tBGLP: {:10}", mean(stats.tot_bank_parallelism, stats.num_write_drains), mean(stats.tot_bankgroup_parallelism, stats.num_write_drains)));
